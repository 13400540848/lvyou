(function($) {
    ...
    $.uedWidget('ued.Dialog', {
	    ...
	    _createDialogDom : function($el, options, _define){
	        ...
	        //判断是否需要生成遮罩
			if(!options.modal){
			    var $dragMask = _define.$dragMask = $('<div style="position: fixed;z-index:9999;top:0;left:0;right:0;bottom:0;display:none;background-color:#000; opacity:0.3; filter:alpha(opacity=30);"></div>');
		...
		_dialogDrag : function(self, _define){
			...
			var mouseMove = function(e){
				_define.left = e.clientX - tempX + (left || 0);
				_define.top = e.clientY - tempY + (top || 0);
		...
		setWidth : function(_width){
			...
			//存储弹窗宽度，以便后期计算位置
			_define.width = $dialogContent.width() + 10;
		},
		...
		setHeight : function(_height){
			...
			//存储弹窗高度，以便后期计算位置
			_define.height = $dialogContent.height() + 50;
		},
		/*
		 * 给提示型弹窗设置提示信息
		...
		open : function(){
			...
			$dialog.show();
			_define.$dragMask && _define.$dragMask.show();
		},
		...
		close : function(){
			...
			$dialog.hide();
			_define.$dragMask && _define.$dragMask.hide();
