...
var PlayRewardAdd = {
	...
	"init": function () {
		...
		if (PlayRewardAdd.Id) {
			...
			HttpUtils.httpGet(Config.WEB_SERVER_API + Config.URI.PLAY_THREE_REWRAD + "/" + PlayRewardAdd.Id, {}, function (data) {
				UIUtils.loaded();
				if (data && data.rows) {
	...
	"saveData": function () {
		...
		HttpUtils.httpPost(Config.WEB_SERVER_API + Config.URI.PLAY_THREE_REWRAD, query, function (data, success) {
			UIUtils.loaded();
			if(success){
				if (parent && parent.PlayRewardManage) {
					parent.PlayRewardManage.search2();
					parent.UIUtils.closeWindow();
				}
			}
		});
