...
var PlayThree = {
  ...
  "init": function () {
    ...
    layui.use(['table', 'element', 'form'], function () {
      ...
      table.render({
        ...
        cols: [[
          { field: 'playTime', title: '期号', align: 'center', width: 88, rowspan: 2 }
          , { field: 'number1', title: '开奖号码', rowspan: 2, width: 60, align: 'center', templet: function (d) { return d.number1 + ' &nbsp; ' + d.number2 + ' &nbsp; ' + d.number3; } }
      ...
      table.render({
        ...
        cols: [[
          { field: 'playTime', title: '期号', width: 100, align: 'center'}
          , { field: 'mode', title: '形态', width: 80, align: 'center', templet: function (d) { return PlayThree.formatMode(d); }}
          , { field: 'statusName', title: '开奖结果', align: 'center', width: 70, templet: function (d) { return PlayThree.formatPlayStatus(d); }}
          , { field: 'number', title: '选号', width: 180, templet: function (d) { return PlayThree.formatSelectNumber(d); } }          
          , { field: 'countNumber', title: '注数', width: 50}
          , { field: 'sumMoney', title: '投注金额', width: 70, templet: function (d) { return FormatUtils.formatDG(d.sumMoney); } }
          , { field: 'createTime', title: '投注时间', width: 150, templet: function (d) { return FormatUtils.parseTimestampToDateTime(d.createTime); } }
          , { field: 'rewardCodeName', title: '中奖奖项', width: 100 }
          , { field: 'rewardMoney', title: '中奖金额', width: 70, templet: function (d) { return FormatUtils.formatDG(d.rewardMoney); } }
  ...
  "initWait": function () {
    ...
    HttpUtils.httpGet(url, {}, function (data) {
      ...
      if (PlayThree.WaitData.nowTime <= PlayThree.WaitData.endTime) {//投注中
  ...
  "initTimerWait":function(){
    ...
    PlayThree.Timer = setInterval(function () {
      ...
      if (PlayThree.SecondTime <= 0) {
        ...
        PlayThree.initWait();
        layer.alert('您好，第 ' + PlayThree.WaitData.playTime + ' 期已截止，当前期是第 ' + (PlayThree.WaitData.playTime + 1) + ' 期，投注时请确认您选择的期号！');
      } else {
        //计算出相差天数
  ...
  "setSureSelectButton": function () {
    if (PlayThree.IsStop) {
      layer.alert('本期投注已截止！', { title: '出错了', icon: 5 });
      return;
    }
